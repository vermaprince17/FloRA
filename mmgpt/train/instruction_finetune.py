...
def main():
    ...
    if args.resume_from_checkpoint is not None:
        ...
        resume_from_step = checkpoint.get("steps", 0)
    ...
    for epoch in range(resume_from_epoch, args.num_epochs):
        ...
        is_resuming = epoch == resume_from_epoch
        start_step = resume_from_step if is_resuming else 0
...
def train_one_epoch(
    ...
    for num_steps, batch in tqdm(
        enumerate(train_dataloader, start= start_step),
        disable=args.rank != 0,
        total=total_training_steps,
        initial=(epoch * num_batches_per_epoch),
    ):
        if is_resuming and num_steps < start_step:
            continue  # Skip steps until the resume step is reached
        ...
        #since training per epoch takes time, dumping ckpts for every step kinda temp ckpt so that we can resume training from these ckpt incase the code
        #fails with in an epoch. Saving ckpoints every 128 steps for num_batches_per_epoch = 4096
        if (num_steps + 1) % (num_batches_per_epoch/32) == 0:
            checkpoint_dict = {
                    "epoch": epoch,
                    "steps": num_steps + 1,
            ...
            checkpoint_dir = f"{args.run_name}/ckpt_per_steps"
            if not os.path.exists(checkpoint_dir):
                os.makedirs(checkpoint_dir)

            # Now save the checkpoint
            checkpoint_path = os.path.join(checkpoint_dir, f"checkpoint_{epoch}_{num_steps + 1}.pt")        
            
            print(f"Saving checkpoint to {checkpoint_path}")
            torch.save(checkpoint_dict, f"{checkpoint_path}")
            
            #  saving every 25 % of steps per eoch
            if (num_steps + 1) % (num_batches_per_epoch/4) == 0 and args.report_to_wandb and args.save_checkpoints_to_wandb:
                wandb.save(f"{args.run_name}/ckpt_per_steps/checkpoint_{epoch}_{num_steps + 1}.pt")
